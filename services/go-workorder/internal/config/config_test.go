...
func TestLoad_AllEnvVarsSet(t *testing.T) {
	t.Setenv("WORKORDER_DATABASE_URL", "postgres://user:pass@localhost:5432/testdb")
	t.Setenv("WORKORDER_PORT", ":9090")
...
func TestLoad_MissingDatabaseURL(t *testing.T) {
	os.Unsetenv("WORKORDER_DATABASE_URL")

	cfg, err := Load()
	if err == nil {
		t.Fatal("expected error when WORKORDER_DATABASE_URL is missing, got nil")
	}
	if cfg != nil {
...
func TestLoad_DefaultValues(t *testing.T) {
	...
	if cfg.Port != ":8085" {
		t.Errorf("Port = %q, want default %q", cfg.Port, ":8085")
	}
...
// - When WORKORDER_DATABASE_URL is empty, Load returns an error
// - When WORKORDER_DATABASE_URL is non-empty, Load returns a valid Config with correct DatabaseURL
//
...
func TestConfigRequiredFieldValidation(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// Apply environment
		if setDB {
			os.Setenv("WORKORDER_DATABASE_URL", dbURL)
		} else {
			os.Unsetenv("WORKORDER_DATABASE_URL")
		}
		if setPort {
			os.Setenv("WORKORDER_PORT", portVal)
		} else {
			os.Unsetenv("WORKORDER_PORT")
		}
		if setRcApi {
		...
		if !setDB {
			...
			if err == nil {
				t.Fatal("expected error when WORKORDER_DATABASE_URL is not set")
			}
			if cfg != nil {
				t.Fatal("expected nil config when WORKORDER_DATABASE_URL is not set")
			}
		} else {
			...
			if setPort {
				...
				if cfg.Port != ":8085" {
					t.Fatalf("Port = %q, want default %q", cfg.Port, ":8085")
				}
			}
