...
- [ ] `AssetCommitment` 已落地
- [ ] `Brand Attestation` 已落地
- [ ] `Platform Attestation` 已落地
- [ ] 当前 V1 验真链路已有稳定集成测试
...
- [ ] 复用现有 `uid / ctr / cmac` 解析与校验逻辑
- [ ] 复用现有 `CTR` replay 检查
- [ ] 不重复造一套标签认证逻辑

### Task 2.2: 接入 `AssetCommitment` 定位
- [ ] 通过 `uid + epoch` 或桥接关系定位 `asset_commitment_id`
- [ ] 若找不到，返回 `ASSET_COMMITMENT_NOT_FOUND`
...
### Task 2.4: 接入状态与风险判断
- [ ] 读取当前状态
- [ ] 结合风控标记输出 `restricted / authentic / incomplete_attestation`
...
- [ ] 记录 `verification_version = v2`
- [ ] 记录 `asset_commitment_id`
- [ ] 记录品牌与平台承诺状态
- [ ] 记录最终结论
...
### Task 4.1: 单元测试
- [ ] 结论判定矩阵测试：
  - [ ] 标签通过 + 双承诺有效 + 正常状态 = `authentic`
  - [ ] 标签通过 + 承诺缺失 = `incomplete_attestation`
  - [ ] 标签通过 + 状态受限 = `restricted`
  - [ ] 标签失败 = `authentication_failed`
...
- [ ] `/verify/v2` 成功上线且不破坏 `/verify`
- [ ] 能明确区分“标签真实”与“终局协议真品成立”
- [ ] 能展示双承诺状态
- [ ] 可作为 Stage 8~10 的协议演进演示入口
